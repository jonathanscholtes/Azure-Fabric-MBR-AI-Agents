...
logger = logging.getLogger("insights-api.routes.presentations")
...
async def _run_presentation_agent(
    openai_client: Any,
    period: str,
    region: str,
) -> dict:
    ...
    response = await openai_client.responses.create(
        ...
        extra_body={"agent_reference": _agent_ref(settings.PRESENTATION_AGENT_NAME)},
    )
...
async def generate_presentation(
    ...
    try:
        agent_result = await asyncio.wait_for(
            _run_presentation_agent(openai_client, period_label, body.region),
            timeout=240.0,
        )
